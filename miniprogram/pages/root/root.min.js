...
Page({
    ...
    onQuery(target) {
        ...
        db.collection("users").where({
            ...
            success: res => {
                wx.hideLoading(), res.data.length ? (this.data.nowTarget = target, this.data.nowTargetId = res.data[0]._id, this.setData({
    ...
    loseFocu(e) {
        ...
        var target, targetU, targetR, index;
        "number" == typeof type && (target = "info.user.[" + type + "].value", 4 === type ? (value = parseInt(value), this.setData({
            [target]: parseInt(value)
        })) : this.setData({
            [target]: value
        })), "string" == typeof type && (targetU = type.charAt(type.length - 2), targetR = type.charAt(type.length - 1), index = parseInt(type.slice(0, type.length - 2)), target = "u" === targetU ? "r" === targetR ? "info.dataU.[" + index + "].right" : "info.dataU.[" + index + "].left" : "r" === targetR ? "info.dataD.[" + index + "].right" : "info.dataD.[" + index + "].left"), this.setData({
            nowINput: "999",
            [target]: value
        })
    },
    ...
    updateInfo(target) {
        ...
        db.collection("users").where({
            ...
            res.data.length ? db.collection("users").doc(res.data[0]._id).update({
    ...
    saveInfo(e) {
        "save" === e.mark.mymark ? this.updateInfo(this.data.info.user[4].value) : this.dataDelete(this.data.nowTarget, this.data.nowTargetId)
    },
    ...
    changeDataType(target) {
        let result1 = {},
            result2 = {},
            result3 = {};
        for (let item of target.user) result1[item.name] = item.value;
        for (let item of target.dataU) item.left && (result2[item.name + "ygLe"] = item.left), result2[item.name + "ygRi"] = item.right;
        for (let item of target.dataD) item.left && (result3[item.name + "jmLe"] = item.left), result3[item.name + "jmRi"] = item.right;
        let rsult4 = {};
        return Object.assign(rsult4, result3, result2, result1), rsult4
    }
});